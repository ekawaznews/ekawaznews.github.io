...
const AI_PROVIDERS = [
  {
    ...
    // Groq (Aug 16, 2026) and moved to Enterprise-only. These are current.
    models: ['openai/gpt-oss-20b', 'meta-llama/llama-4-scout-17b-16e-instruct', 'qwen/qwen3-32b'],
  },
  {
    name: 'Cerebras',
    kind: 'openai',
    ...
    // Cerebras cut its free catalog down sharply — these are the current ones.
  ...
  },
  {
    ...
    // openrouter/free is OpenRouter's own router — it auto-picks whichever
    // free model is currently live, so this never goes stale like a specific
    // model slug would (those rotate weekly on OpenRouter's free tier).
...
{"title":"headline under 85 chars","excerpt":"2 sentence summary under 190 chars","body":"<p>...</p><h2>...</h2><p>...</p>","seoTitle":"SEO title 52-60 chars","seoDesc":"meta description 135-155 chars","tags":["tag1","tag2","tag3","Pakistan"]}`;
  ...
  const maxTokens = type === 'bulletin' ? 1800 : 4500;
  ...
  try {
    ...
    console.log(`  [D] JSON parse failed: ${e.message} — attempting repair`);
    const repaired = repairTruncatedJSON(clean);
    if (repaired) {
      console.log('  [D] Repair succeeded — using recovered article');
      return repaired;
    }
    console.log('  [D] Repair failed. Skipping article.');
    return null;
  }
}

// If the AI response got cut off mid-string (rare, but happens with any
// provider under load), try to salvage what's usable instead of throwing
// the whole article away. Finds the last fully-closed field and rebuilds
// a valid JSON object from just the complete fields.
function repairTruncatedJSON(text) {
  try {
    const fields = ['title', 'excerpt', 'body', 'seoTitle', 'seoDesc'];
    const result = {};
    ...
    // tags array — grab whatever complete quoted strings exist inside it
    const tagsMatch = text.match(/"tags"\s*:\s*\[([^\]]*)\]/s);
    result.tags = tagsMatch ? (tagsMatch[1].match(/"([^"]+)"/g) || []).map(s => s.replace(/"/g, '')) : [];
    // A repaired article needs at least a title and a real body to be worth publishing
    if (result.title && result.body && result.body.length > 200) return result;
    return null;
  } catch (_) {
    return null;
  }
